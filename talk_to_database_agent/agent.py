...
root_agent = Agent(
    ...
    """,
    tools=[AgentTool(agent=bigquery_agent)] + MATH_TOOLS,
...
app = App(
    ...
    # Compactação de histórico. Dispara por contagem de tokens, e não a cada N
    # turnos: o que enche a janela aqui não é conversar muito, e sim uma query
    # que devolve muitas linhas — `sql_results` inteiro vai para o histórico.
    # Os dois campos são obrigatórios em par; 100k ≈ 10% da janela de 1M do
    # gemini-3.6-flash, então resume bem antes de haver qualquer risco.
    # Sem `summarizer`, o ADK cria um LlmEventSummarizer com o modelo do agente.
